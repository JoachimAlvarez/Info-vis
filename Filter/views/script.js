...
$('#applyFilter').on('click', e => {  
  ...
  if (compounds.length === 0) {
    alert('No filter was entered, please select a compound to filter')
  }
  else {
    hideElement(modal)
    ...
    $.ajax({
    	...
    	url: 'http://localhost:4000/getFoodByCompounds',
    	data: {compounds : JSON.stringify(compounds), number: compounds.length},
      timeout: 0,
    	traditional: true,
    	success: data => {
    	  DATA = data.json;
...
function showFoodInformation(d) {
  ...
  if (min_grade && max_grade) {
  	html += '<p><b>Dish Grade Range: </b>'+gradesSpan(min_grade,max_grade)+'</p>';
  }
  else {
  	html += '<p><b>Dish Grade Range: </b> No information </p>';
  }
...
function showGroupInformation(d) {
  ...
  var div = document.createElement('div');
  let html = "";
  html += '<p><b>Dish Grade Rage: </b>'+gradesSpan(min_grade,max_grade)+'</p>';
