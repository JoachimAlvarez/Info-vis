...
$('#applyFilter').on('click', e => {  
  ...
  $.ajax({
  	...
  	url: 'http://localhost:4001/getFoodByCompounds',
...
function showDishInformation(d) {
  ...
  if (grade) {
    gradespan += '<span>';
    for (i = 0; i < Math.floor(grade); i ++) {
      gradespan += '<i class="fas fa-star"></i>';
    }
    if (grade % 1 != 0) {
      gradespan += '<i class="fas fa-star-half-alt"></i>';
    }
    for (i = 0; i < (5-Math.ceil(grade)); i++) {
      gradespan += '<i class="far fa-star"></i>';
    }
    gradespan += '</span>';
  } 
  else {
    gradespan = 'grade not available';
  }

  var div = document.createElement('div');
  let html = '';
  html += '<p><b>Food Group: </b>'+parent_name+'</p>';
  html += '<p><b>Grade: </b>'+gradespan+'</p>';
  html += '<p>'+description+'</p>';
  div.innerHTML = html;
  document.getElementById('info').appendChild(div);
}
...
$.ajax({
  type: 'GET',
  contentType: 'application/json',
  url: 'http://localhost:4001/getFoodGroups',
  success: data => {
    DATA = data.json;
    console.log(DATA)
    DrawVisualisation(data.json, data.names)
